...
class DeviceArchiveControllerTests {
    ...
    void exportsDeviceQrCodeAfterPasswordVerification() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/log-entries?action=DEVICE_ARCHIVE_EXPORT&keyword=RDVP-DEVICE-0001")
        ...
        mockMvc.perform(get("/api/v1/log-entries?action=DEVICE_ARCHIVE_EXPORT&keyword=RDVP-DEVICE-9999")
                        .header("Authorization", "Bearer " + managerToken))
                .andExpect(status().isOk())
    ...
    @Test
    void verifiesDeviceArchiveDetailExportAndRecordsLogEntry() throws Exception {
        String token = login("archiveadmin", "password");
        verifyPassword(token, "password");
