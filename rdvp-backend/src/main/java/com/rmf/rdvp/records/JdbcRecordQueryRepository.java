...
@Repository
@Profile("!test")
public class JdbcRecordQueryRepository implements RecordQueryRepository {

    private final NamedParameterJdbcTemplate jdbcTemplate;

    public JdbcRecordQueryRepository(NamedParameterJdbcTemplate jdbcTemplate) {
    ...
    public RecordListResponse queryArchiveRecords(
        ...
        List<String> auditConditions = new ArrayList<>();
        auditConditions.add("al.action IN ('DEVICE_ARCHIVE_QUERY', 'DEVICE_ARCHIVE_EXPORT')");

        if (!normalizedType.isBlank()) {
            if (isArchiveRequestType(normalizedType)) {
                requestConditions.add("cr.request_type = :type");
                auditConditions.add("1 = 0");
            } else if (isArchiveAuditType(normalizedType)) {
                requestConditions.add("1 = 0");
                auditConditions.add("al.action = :type");
            } else {
                requestConditions.add("1 = 0");
                auditConditions.add("1 = 0");
            }
        ...
        addTimeConditions(auditConditions, "al.occurred_at", timeRange, parameters);
        ...
        String auditSql = """
                ...
                FROM audit_logs al
                """
                + buildWhereClause(auditConditions);

        return queryUnion(requestSql + " UNION ALL " + auditSql, parameters);
    }
    ...
    public RecordListResponse queryReviewRecords(
        ...
        if (hasKeyword(keyword)) {
            ...
            operationConditions.add("""
                    (
                        ...
                        OR COALESCE(review_op.display_name, review_op.username, op.display_name, op.username, orr.review_operator_id, orr.operator_id) ILIKE :keyword
                    )
                    """);
        }
        ...
        String operationSql = """
                SELECT
                    ...
                    COALESCE(review_op.display_name, review_op.username, op.display_name, op.username, orr.review_operator_id, orr.operator_id) AS operator_name,
        ...
        return queryUnion(archiveSql + " UNION ALL " + operationSql, parameters);
    }
    ...
    private RecordListResponse queryUnion(String unionSql, MapSqlParameterSource parameters) {
    ...
    private RecordItemResponse mapRecordItem(ResultSet rs, int rowNum) throws SQLException {
    ...
    private boolean isArchiveAuditType(String type) {
