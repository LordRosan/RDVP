...
public class BootstrapUserStore implements UserAccountRepository {
    ...
    public BootstrapUserStore(PasswordEncoder passwordEncoder, RdvpRuntimeProperties runtimeProperties) {
        ...
        var bootstrapUsers = BootstrapUserDefinitions.createUsers(passwordEncoder, defaultPassword);

        this.usersById = bootstrapUsers.stream().collect(Collectors.toUnmodifiableMap(BootstrapUser::id, Function.identity()));
        this.usersByUsername = bootstrapUsers.stream()
