...
public class JdbcUserAccountRepository implements UserAccountRepository {
    ...
    private Set<PermissionCode> findPermissions(String userId) {
        return jdbcTemplate.queryForList(
                        "SELECT permission_code FROM user_permissions WHERE user_id = :userId",
                        Map.of("userId", userId),
                        String.class)
                .stream()
    ...
    private void replaceRoles(BootstrapUser user) {
        jdbcTemplate.update("DELETE FROM user_roles WHERE user_id = :userId", Map.of("userId", user.id()));
    ...
    private void replacePermissions(BootstrapUser user) {
        jdbcTemplate.update("DELETE FROM user_permissions WHERE user_id = :userId", Map.of("userId", user.id()));
