...
@Service
public class DeviceArchiveChangeRequestService {
    ...
    public DeviceArchiveChangeRequestService(
    ...
    @Transactional
    public DeviceArchiveChangeRequest create(
    ...
    public DeviceArchiveChangeRequest create(
            ...
            AuthenticatedUser applicant) {
        DeviceArchiveChangeRequestType requestType = type == null ? DeviceArchiveChangeRequestType.UPDATE : type;
        try {
    ...
    public DeviceArchiveChangeRequestPage list(
            String status,
            String deviceCode,
            String applicantId,
            int page,
            int pageSize) {
        DeviceArchiveChangeRequestStatus parsedStatus = parseStatus(status);
        ...
        DeviceArchiveChangeRequestPage pageResult = changeRequestRepository.list(new DeviceArchiveChangeRequestQuery(
                parsedStatus,
        ...
        return new DeviceArchiveChangeRequestPage(
                pageResult.items().stream().map(this::enrichApplicantName).toList(),
                pageResult.total());
    }

    @Transactional
    public DeviceArchiveChangeRequest review(
            String requestId,
        ...
        DeviceArchiveChangeRequest request = null;
        try {
            normalizedRequestId = normalizeRequiredId(requestId, "requestId");
            request = changeRequestRepository.findById(normalizedRequestId)
                    .orElseThrow(() -> new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_NOT_FOUND));

            if (request.status() != DeviceArchiveChangeRequestStatus.PENDING_REVIEW) {
                throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_ALREADY_REVIEWED);
            }
            ...
            if (decision == DeviceArchiveReviewDecision.REJECTED && normalizedComment.isBlank()) {
                throw new BusinessException(
                        ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_INVALID,
            ...
            DeviceArchiveChangeRequest reviewed = enrichApplicantName(changeRequestRepository.findById(request.id())
    ...
    private void recordChangeRequestFailure(
            DeviceArchiveChangeRequestType requestType,
            String deviceId,
            String deviceCode,
            AuthenticatedUser applicant,
        ...
        auditLogService.recordFailure(
                ...
                requestType == DeviceArchiveChangeRequestType.CREATE ? null : normalizeAuditTarget(deviceId),
    ...
    private void recordChangeReviewFailure(
            String requestId,
            DeviceArchiveChangeRequest request,
            AuthenticatedUser reviewer,
            BusinessException exception) {
        auditLogService.recordFailure(
                AuditAction.DEVICE_ARCHIVE_REVIEW,
    ...
    private DeviceArchiveChangeRequest createUpdateRequest(
            String deviceId,
            String reason,
            Map<String, DeviceArchiveChangeValue> changes,
            OffsetDateTime initiatedAt,
            AuthenticatedUser applicant) {
        requirePermission(applicant, PermissionCode.ARCHIVE_DEVICE_CHANGE_REQUEST_CREATE);
        ...
        if (changeRequestRepository.hasPendingByDeviceId(device.id())) {
            throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_LOCKED);
        }

        if (changeRequestRepository.findActiveFreezeUntil(device.id(), now).isPresent()) {
            throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_FROZEN);
        }

        Map<String, DeviceArchiveChangeValue> normalizedChanges = normalizeAndValidateUpdateChanges(device, changes);
        DeviceArchiveChangeRequest created = createRequest(new DeviceArchiveChangeRequestCreate(
                newRequestId(),
    ...
    private DeviceArchiveChangeRequest createArchiveCreateRequest(
            String deviceCode,
            String reason,
            Map<String, DeviceArchiveChangeValue> changes,
            OffsetDateTime initiatedAt,
        ...
        Map<String, DeviceArchiveChangeValue> normalizedChanges = normalizeAndValidateCreateChanges(changes);
        DeviceArchiveChangeRequest created = createRequest(new DeviceArchiveChangeRequestCreate(
                newRequestId(),
                DeviceArchiveChangeRequestType.CREATE,
                null,
                normalizedDeviceCode,
                applicant.id(),
                "NEW",
    ...
    private DeviceArchiveChangeRequest createArchiveDeleteRequest(
            String deviceId,
            String reason,
            OffsetDateTime initiatedAt,
            AuthenticatedUser applicant) {
        ...
        DeviceArchiveChangeRequest created = createRequest(new DeviceArchiveChangeRequestCreate(
                newRequestId(),
                DeviceArchiveChangeRequestType.DELETE,
                device.id(),
                device.deviceCode(),
                applicant.id(),
                device.status(),
    ...
    private DeviceArchiveChangeRequest createRequest(DeviceArchiveChangeRequestCreate request) {
        try {
    ...
    private void verifyApprovalBaseline(DeviceArchive currentDevice, Map<String, DeviceArchiveChangeValue> changes) {
        for (Map.Entry<String, DeviceArchiveChangeValue> entry : changes.entrySet()) {
            String field = normalizeSupportedField(entry.getKey());
            DeviceArchiveChangeValue value = requireChangeValue(entry.getValue());
    ...
    private BusinessException createConstraintConflict(DeviceArchiveChangeRequestCreate request) {
        if (request.type() == DeviceArchiveChangeRequestType.CREATE) {
            return new BusinessException(ErrorCode.DEVICE_CODE_DUPLICATED);
        }

        return new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_LOCKED);
    }

    private Map<String, DeviceArchiveChangeValue> normalizeAndValidateUpdateChanges(
            DeviceArchive device,
            Map<String, DeviceArchiveChangeValue> changes) {
        if (changes == null || changes.isEmpty()) {
            throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_INVALID);
        }

        Map<String, DeviceArchiveChangeValue> normalizedChanges = new HashMap<>();
        for (Map.Entry<String, DeviceArchiveChangeValue> entry : changes.entrySet()) {
            String field = normalizeSupportedField(entry.getKey());
            DeviceArchiveChangeValue value = requireChangeValue(entry.getValue());
            ...
            if (!newValue.equals(currentValue)) {
                normalizedChanges.put(field, new DeviceArchiveChangeValue(currentValue, newValue));
            }
        }
    ...
    private Map<String, DeviceArchiveChangeValue> normalizeAndValidateCreateChanges(Map<String, DeviceArchiveChangeValue> changes) {
        if (changes == null || changes.isEmpty()) {
            throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_INVALID);
        }
        ...
        for (Map.Entry<String, DeviceArchiveChangeValue> entry : changes.entrySet()) {
            ...
            if (!newValue.isBlank()) {
                normalizedChanges.put(field, new DeviceArchiveChangeValue("", newValue));
            }
        }
    ...
    private Map<String, DeviceArchiveChangeValue> buildDeleteSnapshot(DeviceArchive device) {
        Map<String, DeviceArchiveChangeValue> snapshot = new HashMap<>();
        snapshot.put("name", new DeviceArchiveChangeValue(normalizeText(device.name()), ""));
        snapshot.put("model", new DeviceArchiveChangeValue(normalizeText(device.model()), ""));
        snapshot.put("manufacturer", new DeviceArchiveChangeValue(normalizeText(device.manufacturer()), ""));
        snapshot.put("location.address", new DeviceArchiveChangeValue(normalizeText(device.address()), ""));
        return Map.copyOf(snapshot);
    }

    private DeviceArchiveCreate buildArchiveCreate(DeviceArchiveChangeRequest request, String reviewerId, OffsetDateTime reviewedAt) {
    ...
    private DeviceArchiveUpdate buildArchiveUpdate(
        ...
        for (Map.Entry<String, DeviceArchiveChangeValue> entry : changes.entrySet()) {
            switch (entry.getKey()) {
                ...
                default -> throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_INVALID);
            }
        }
    ...
    private DeviceArchiveChangeValue requireChangeValue(DeviceArchiveChangeValue value) {
        if (value == null) {
            throw new BusinessException(ErrorCode.DEVICE_ARCHIVE_CHANGE_REQUEST_INVALID);
        }
    ...
    private String requireCreatedValue(DeviceArchiveChangeRequest request, String field) {
    ...
    private String createdValue(DeviceArchiveChangeRequest request, String field) {
        DeviceArchiveChangeValue value = request.changes().get(field);
    ...
    private String formatRequestType(DeviceArchiveChangeRequestType requestType) {
        return switch (requestType) {
            case UPDATE -> "变更";
            case CREATE -> "新增";
            case DELETE -> "删除";
    ...
    private DeviceArchiveChangeRequestStatus parseStatus(String status) {
        if (status == null || status.isBlank()) {
            return null;
        }

        try {
            return DeviceArchiveChangeRequestStatus.valueOf(status.trim().toUpperCase());
    ...
    private DeviceArchiveChangeRequest enrichApplicantName(DeviceArchiveChangeRequest request) {
        String applicantName = userStore.findById(request.applicantId())
                .map(user -> user.displayName())
                .orElse(request.applicantId());
        return new DeviceArchiveChangeRequest(
                request.id(),
                request.type(),
                request.deviceId(),
                request.deviceCode(),
