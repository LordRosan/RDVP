...
@Repository
@Profile("!test")
public class JdbcDeviceArchiveChangeRequestRepository implements DeviceArchiveChangeRequestRepository {

    private static final TypeReference<Map<String, DeviceArchiveChangeValue>> CHANGE_MAP_TYPE = new TypeReference<>() {
    };

    private final NamedParameterJdbcTemplate jdbcTemplate;
    private final ObjectMapper objectMapper;

    public JdbcDeviceArchiveChangeRequestRepository(
    ...
    public Optional<DeviceArchiveChangeRequest> findById(String id) {
        List<DeviceArchiveChangeRequest> results = jdbcTemplate.query(
    ...
    public DeviceArchiveChangeRequestPage list(DeviceArchiveChangeRequestQuery query) {
        ...
        List<DeviceArchiveChangeRequest> items = jdbcTemplate.query(
        ...
        Long total = jdbcTemplate.queryForObject(
                ...
                parameters,
                Long.class);
        return new DeviceArchiveChangeRequestPage(items, total == null ? 0 : total);
    }
    ...
    public void create(DeviceArchiveChangeRequestCreate request) {
        jdbcTemplate.update(
                """
                        INSERT INTO device_archive_change_requests (
                            id,
                            request_type,
                            device_id,
                            target_device_code,
    ...
    public boolean applyApprovedReview(
        ...
        int updatedRequest = jdbcTemplate.update(
                """
                        UPDATE device_archive_change_requests
                        SET status = 'APPROVED',
    ...
    private String buildWhereClause(DeviceArchiveChangeRequestQuery query, MapSqlParameterSource parameters) {
    ...
    private DeviceArchiveChangeRequest mapChangeRequest(ResultSet resultSet, int rowNumber) throws SQLException {
        Map<String, DeviceArchiveChangeValue> changes = readChanges(resultSet.getString("changes"));
        return new DeviceArchiveChangeRequest(
                resultSet.getString("id"),
                DeviceArchiveChangeRequestType.valueOf(resultSet.getString("request_type")),
                ...
                null,
                DeviceArchiveChangeRequestStatus.valueOf(resultSet.getString("status")),
    ...
    private String resolveDeviceName(String deviceName, Map<String, DeviceArchiveChangeValue> changes) {
        if (deviceName != null && !deviceName.isBlank()) {
            return deviceName;
        }

        DeviceArchiveChangeValue nameChange = changes.get("name");
    ...
    private String writeChanges(Map<String, DeviceArchiveChangeValue> changes) {
        try {
    ...
    private Map<String, DeviceArchiveChangeValue> readChanges(String changesJson) {
        try {
